...
from production import P13


class TestProduction13(unittest.TestCase):

    def is_any_mapping_feasible(self, graph: Graph):
        p13 = P13()
        mapping_gen = graph.generate_subgraphs_isomorphic_with(p13.get_lhs())
        return any(p13.is_mapping_feasible(graph, mapping) for mapping in mapping_gen)
    ...
    def test_production_cannot_be_applied_if_one_hanging_node_is_missing(self):
        ...
        node_5 = Node(NodeAttrs('v', 0.5, 0, True))
        node_6 = Node(NodeAttrs('v', 0.5, 1, True))
        nodes = [node_0, node_5, node_1, node_2, node_3, node_6, node_4]
        graph.add_node_collection(nodes)
    ...
    def test_production_cannot_be_applied_if_one_hanging_node_is_in_wrong_place(self):
        ...
        node_6 = Node(NodeAttrs('v', 0.5, 1, True))
        node_7 = Node(NodeAttrs('v', 0, 0.5, True))        
        nodes = [node_0, node_1, node_5, node_2, node_3, node_6, node_4, node_7]
        graph.add_node_collection(nodes)
