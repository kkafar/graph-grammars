...
class TestProduction10(unittest.TestCase):
    ...
    def test_production_can_be_applied_on_more_complex_graph(self):
        ...
        v_node = graph.split_edge_with_vnode(edge=(node_1.handle, node_2.handle), node_flag=True)
        edges = graph.get_real_edges()
        
        res = P10()(graph)
        
        self.assertTrue(res)
        self.assertFalse(graph.node_attrs(v_node.handle).flag)
    ...
    def test_production_cannot_be_applied_if_no_hanging(self):
        ...
        graph.split_edge_with_vnode(edge=(node_1.handle, node_2.handle), node_flag=False)

        self.assertFalse(P10()(graph))
    ...
    def test_production_cannot_be_applied_if_R_false(self):
        ...
        graph.split_edge_with_vnode(edge=(node_1.handle, node_2.handle), node_flag=True)
        self.assertFalse(P10()(graph))
