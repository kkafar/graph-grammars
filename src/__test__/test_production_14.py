...
from production import P14


class TestProduction14(unittest.TestCase):

    def is_any_mapping_feasible(self, graph: Graph):
        p14 = P14()
        mapping_gen = graph.generate_subgraphs_isomorphic_with(p14.get_lhs())
        return any(p14.is_mapping_feasible(graph, mapping) for mapping in mapping_gen)
