...
class P15(Production):
    ...
    def __create_lhs(self) -> Graph:
        ...
        node_9 = Node(NodeAttrs('v', 1.25, 0.25, True), 9)
        nodes = [node_0, node_5, node_1, node_9, node_2, node_8, node_3, node_6, node_4, node_7]
        graph.add_node_collection(nodes)

        graph.add_p_hyperedge(corner_nodes, EdgeAttrs('p', True), 10)
    ...
    def apply_with_mapping(self, graph: Graph, mapping: Dict[NodeHandle, NodeHandle]):
        ...
        hanging_nodes = [graph.node_for_handle(rev_mapping[i]) for i in [5, 6, 7, 8, 9]]
